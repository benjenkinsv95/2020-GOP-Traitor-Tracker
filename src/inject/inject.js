...
const getInnerMostTraitorElements = selector => {
	const allTraitorElements = $(selector).find(allTraitorsCssSelector)
...
const markTraitors = (node, text, onlyHighlight) => {
	...
	// 	console.log('num matches', matches.size)
...
const markNewContent = function(mutationsList, observer) {
    for(let mutation of mutationsList) {
        for (let node of mutation.addedNodes) {
			...
			// find the elements that have state, firstName, lastName, and job, without a similar element within them
			const innerMostNode = getInnerMostTraitorElements(node)

            if (innerMostNode.length > 0) {
				console.log('node', node)
				console.log('innerMostNode', innerMostNode)
				const text = $(innerMostNode).text();

				// use setTimeout to avoid blocking the UI
				setTimeout(() => {
					markTraitors(innerMostNode, text, true)
				}, 0)
...
chrome.extension.sendMessage({}, function(response) {
	...
	if (document.readyState === "complete") {
		...
		$(() => {
			// const allTraitorElements = $('body').find(allTraitorsCssSelector)
			// const innermostTraitorElements = allTraitorElements.not(allTraitorElements.has(allTraitorElements))
			getInnerMostTraitorElements('body')
			  .each(function() {
				
				const jQueryNode = $(this)
				// const innerMostNode = jQueryNode.is(allTraitorsCssSelector) && jQueryNode.not(`:has(${allTraitorsCssSelector})`).length === 0 ? jQueryNode: jQueryNode.find(allTraitorsCssSelector).not(`:has(${allTraitorsCssSelector})`)
